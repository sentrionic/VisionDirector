...
import 'package:vision_director/presentation/steps/edit/trimmer/thumbnail_viewer.dart';
import 'package:vision_director/presentation/steps/edit/trimmer/trim_editor_painter.dart';
...
class TrimEditor extends StatefulWidget {
  ...
  const TrimEditor({
    ...
    this.maxVideoLength = Duration.zero,
    this.circleSize = 5.0,
    this.borderWidth = 3,
...
class _TrimEditorState extends State<TrimEditor> with TickerProviderStateMixin {
    ...
    widget.trimmer.eventStream.listen((event) {
      if (event == TrimmerEvent.initialized) {
        ...
        setState(() {
          ...
          if (widget.maxVideoLength > Duration.zero &&
  ...
  Future<void> _initializeVideoController() async {
    if (_videoFile != null) {
      ...
      final ThumbnailViewer viewer = ThumbnailViewer(
            ...
            SizedBox(
              ...
              child: Padding(
                ...
                child: Row(
                  ...
                  children: [
                    Text(
                      getDurationString(_videoStartPos ~/ 1000),
                      style: widget.durationTextStyle,
                    ),
                    if (videoPlayerController.value.isPlaying)
                      Text(
                        getDurationString(_currentPosition ~/ 1000),
                        style: widget.durationTextStyle,
                      )
                    else
                      Container(),
                    Text(
                      getDurationString(_videoEndPos ~/ 1000),
                      style: widget.durationTextStyle,
                    ),
                  ],
                ),
