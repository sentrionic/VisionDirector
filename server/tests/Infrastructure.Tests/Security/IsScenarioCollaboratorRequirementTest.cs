...
public class IsScenarioCollaboratorRequirementTest : TestBase
{
    ...
    public async Task Handle_WhenCalledWithIdOfUserThatIsACollaborator_ShouldSucceed()
    {
        ...
        var requirement = new IsScenarioCollaboratorRequirement();

        var authContext =
            new AuthorizationHandlerContext(new List<IAuthorizationRequirement> { requirement }, user, null);

        var authHandler = new IsScenarioCollaboratorRequirementHandler(_dataContext, mockHttpContextAccessor.Object);
